...
import {SubmitReqUploadedNews,setfiltertype,handleuploadSubmitdata,setresult1,setNewsNow} from "../actions/svpcetuploadverifyactions";
import {
...
//export class App extends React.Component {   commenting because below redux will take care of attacting
 class svpcetverify extends React.Component {
 ...
 this.props.setNewsNow(target.checked);
  ...
  serverRequestNews (){
    ...
    var WORKSPACE_ID = "HOME_WORKSPACE";
  		var FUNCTION_ID = "GET_UPLOADED_SVPCET_NEWS";
  ...
  handleuploadSubmitdata (item){
    ...
    var WORKSPACE_ID = "HOME_WORKSPACE";
      var FUNCTION_ID = "SEND_SVPCETDATA_NEWS_WORKSPACE";
  ...
  var dande = 'ServerData=' + JSON.stringify(json);
   ...
   this.props.handleuploadSubmitdata(dande);
    ...
    items.map((item) => {
      var type= "";
      if (item.NEWS_TYPE == "Y"){
        {/*TYPE 1: ONLY Image (Notify Image)*/}
      type=  "TYPE 1: ONLY Image (Notify Image)";
      }
    else  if (item.NEWS_TYPE == "N"){
        {/*TYPE 1: ONLY Image (Notify Image)*/}
      type=  "TYPE 2: ONLY CONTENT(Notify header display)";
      }
    else  if (item.NEWS_TYPE == ""){
        {/*TYPE 1: ONLY Image (Notify Image)*/}
      type=  "TYPE 3: IMAGE WITH CONTENT ( Notify header , Notify description , Notify Image)";
      }
news.push(
  <Row>
 ...
 <img src={item.NEWS_IMAGE} style= {{ height: '100%',width:'100%',padding:'10px'}}/ >
  ...
  <p><b>Notify Header</b></p>
  <h5 className='home-news-details-h5'>{item.NEWS_HEADER}</h5>
  <p><b>Notify Details</b></p>
  <p >{item.NEWS_DETAILS}</p>

  <p><b>Notify Type</b></p>
<p >{type}</p>
  <p className='home-news-p'><b>UPDATED DATE:</b><span style={{color:'#df4031'}}>{item.POSTED_DATE} </span></p>
...
export default connect(mapStateToProps,mapDispatchToProps)(svpcetverify);
